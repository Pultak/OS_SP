...
bool kiv_os_rtl::Open_File(const char* file_name, kiv_os::NOpen_File file_open, kiv_os::NFile_Attributes file_attribute, kiv_os::THandle &file_handle_ret) {
	...
	const bool result = kiv_os::Sys_Call(regs);
	file_handle_ret = static_cast<uint16_t>(regs.rax.r);
	return result;
}

bool kiv_os_rtl::Seek(kiv_os::THandle file_handle, const uint16_t position, kiv_os::NFile_Seek file_seek_pos, kiv_os::NFile_Seek file_seek_op, uint64_t &position_ret){
...
bool kiv_os_rtl::Get_File_Attribute(char* file_name, uint64_t &file_attribute_ret) {
...
bool kiv_os_rtl::Create_Thread(kiv_os::TThread_Proc thread_name, const char* argument, kiv_os::THandle stdin_handle, kiv_os::THandle stdout_handle, kiv_os::THandle& thread_handle_ret) {
	kiv_hal::TRegisters regs = Prepare_SysCall_Context(kiv_os::NOS_Service_Major::Process, static_cast<uint8_t>(kiv_os::NOS_Process::Clone));
...
bool kiv_os_rtl::Wait_For(kiv_os::THandle* handles_to_wait, uint16_t num_of_handles, kiv_os::THandle &handle_signal_ret) {
	...
	const bool result = kiv_os::Sys_Call(regs);
	handle_signal_ret = static_cast<uint16_t>(regs.rax.l);
	return result;
}
